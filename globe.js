...
// Copyright (c) 2015 Mike van Rossum - modified 2018 Bowei Han
...
var Globe = function Globe(container, urls) {
  ...
  var render = function() {
    ...
    // rotation.x +=
    //   (target.x - rotation.x) * 0.1 > 0 ? (target.x - rotation.x) * 0.1 : 0.001;
    rotation.y += (target.y - rotation.y) * 0.1;
    // rotation.y +=
    //   (target.y - rotation.y) * 0.1 > 0 ? (target.y - rotation.y) * 0.1 : 0.001;
  ...
  var set3dPosition = function(mesh, coords) {
    if (!coords) coords = mesh.userData;
  ...
  var createLevitatingBlock = function(properties) {
    ...
    block.scale.z = properties.magnitude;
